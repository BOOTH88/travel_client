...
function makeTop(travel_username, username) {
    //blog_username 当前访问的博客的作者
    //username   登陆的用户

    //博客作者-用户信息url
    var user_info_url = '/' + travel_username + '/' + 'info'
        //登陆用户发博客url
    if (username) {
        var food_share_url = '/' + username + '/' + 'food/share'
    } else {
        //没有登陆状态直接去登陆
        var food_share_url = '/login'
    }

    //访问博主的博客文章
    var user_foods_url = '/' + travel_username + '/' + 'foods'

    var header_body = ''
    header_body += '<header id="header">';
    header_body += '<div class="menu">';
    header_body += ' <nav class="nav" id="topnav"> ';
    header_body += '<h1 class="logo"><a href="/index"> ' + travel_username + '的美食</a></h1>';
    header_body += '<li><a href="/index">网站首页</a></li>';
    header_body += '<li>';
    header_body += '<a href=' + '"' + user_foods_url + '"' + '>文章列表</a>';
    header_body += '</li>';
    header_body += '<li><a href="photo.html">我的美食</a> </li>';
    header_body += '<li><a href=' + '"' + user_info_url + '"' + '>关于我</a> </li>';
    header_body += '<li><a href=' + '"' + food_share_url + '"' + '>发布·美食</a> </li>';
    header_body += '</nav>';
    header_body += '</div>';
    if (username) {
        header_body += '<li><a href= /' + username + '/change_info id="change_info" target="_blank">编辑</a></li>';
